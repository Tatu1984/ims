import * as repo from "@/backend/repositories/license.repository";
...
export async function getAll() {
  ...
  return records.map((r) => ({
    ...r,
    available: r.totalLicenses - r.inUse,
    compliancePct:
      r.totalLicenses > 0
        ? Math.round(Math.min((r.inUse / r.totalLicenses) * 100, 100))
        : 0,
  }));
}
...
export async function getById(id: string) {
  ...
  if (!record) throw new AppError(404, "License not found");
  return record;
}
